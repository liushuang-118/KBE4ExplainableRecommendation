...
class Tensorflow_data:
    def __init__(self, data_path, input_train_dir, set_name):
        ...
        self.review_info = []
        self.review_text = []
        with gzip.open(input_train_dir + set_name + '.txt.gz', 'rb') as fin:
            for line in fin:
        ...
        with gzip.open(data_path + 'related_product.txt.gz', 'rb') as fin:
            for line in fin:
                self.related_product_ids.append(line.decode('utf-8').strip())
        self.related_product_size = len(self.related_product_ids)

        self.brand_ids = []
        with gzip.open(data_path + 'brand.txt.gz', 'rb') as fin:
            for line in fin:
                self.brand_ids.append(line.decode('utf-8').strip())
        self.brand_size = len(self.brand_ids)

        self.category_ids = []
        with gzip.open(data_path + 'category.txt.gz', 'rb') as fin:
            for line in fin:
                self.category_ids.append(line.decode('utf-8').strip())
        ...
        for name in knowledge_file_dict:
            ...
            with gzip.open(knowledge_file_dict[name], 'rb') as fin:
                for line in fin:
                    line = line.decode('utf-8').strip()
                    knowledge_list = []
                    arr = line.split(' ')
                    for x in arr:
                        if len(x) > 0:
                            x = int(x)
                            knowledge_list.append(x)
                            self.knowledge[name]['distribute'][x] += 1
                    self.knowledge[name]['data'].append(knowledge_list)
        ...
        print("Data statistic: vocab %d, review %d, user %d, product %d\n" % 
              (self.vocab_size, self.review_size, self.user_size, self.product_size))

    # -------------------- 保留原方法，修改 xrange -> range --------------------
    def sub_sampling(self, subsample_threshold):
        ...
        for i in range(self.vocab_size):
            if self.vocab_distribute[i] == 0:
                continue
            self.sub_sampling_rate[i] = min((np.sqrt(float(self.vocab_distribute[i]) / threshold) + 1) * threshold / float(self.vocab_distribute[i]), 1.0)
    ...
    def read_train_product_ids(self, data_path):
        ...
        with gzip.open(data_path + 'train.txt.gz', 'rb') as fin:
            for line in fin:
    ...
    def print_entity_list(self, relation_name, entity_name, entity_scores, rank_cut, remove_map):
        ...
        sorted_entity_idxs = sorted(range(len(entity_scores)), key=lambda k: entity_scores[k], reverse=True)
        entity_rank_list = []
        entity_rank_scores = []
        rank = 0
